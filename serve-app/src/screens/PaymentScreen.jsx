// PaymentScreen.jsx — pay for a court reservation. Apple Pay, Credit/Debit
// card, and TELDA (the methods Egyptians actually use). On success the court is
// secured in the shared store (so it shows as booked everywhere) and added to
// the player's bookings.
...
export default function PaymentScreen({ courtNo, time, endTime, price }) {
  const { nav } = useNav();
  ...
  function pay() {
    setPhase('processing');
    setTimeout(() => {
      store.bookCourt(courtNo, { court: courtNo, type: court?.type, day: 'Today', time, endTime, price, method, status: 'confirmed' });
      setPhase('done');
    }, 1100);
  }
      ...
      >
        {phase === 'done' ? (
          <div className="sq-fade-up" style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '0 32px', gap: 18 }}>
            ...
            <div>
              <h1 className="sq-display" style={{ margin: 0, fontSize: 24, fontWeight: 700, letterSpacing: '-0.02em' }}>Court secured</h1>
              <p style={{ margin: '8px 0 0', color: 'var(--sq-text-2)', fontSize: 14 }}>Court {courtNo} · today {time}–{endTime}</p>
            </div>
            <div className="sq-card" style={{ width: '100%', padding: 16, display: 'flex', flexDirection: 'column', gap: 8, textAlign: 'left' }}>
              {[['Court', `Court ${courtNo} · ${court?.type || ''}`], ['Time', `${time} – ${endTime}`], ['Paid', `EGP ${price}`], ['Method', METHODS.find((m) => m.id === method)?.label]].map(([k, v]) => (
            ...
            <div style={{ display: 'flex', gap: 10, width: '100%' }}>
              ...
              <button className="sq-btn-gold" style={{ flex: 1, padding: '13px', fontSize: 13.5 }} onClick={() => nav.switchTab('clubs')}>Back to club</button>
            </div>
          </div>
        ) : phase === 'processing' ? (
          <div style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16, color: 'var(--sq-text-2)' }}>
            <motion.div animate={{ rotate: 360 }} transition={{ duration: 0.8, repeat: Infinity, ease: 'linear' }}
              style={{ width: 40, height: 40, borderRadius: '50%', border: '3px solid var(--sq-border-2)', borderTopColor: 'var(--sq-gold)' }} />
            <span style={{ fontSize: 14 }}>Securing your court…</span>
          </div>
        ) : (
          <div style={{ padding: '8px 20px 24px', display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div className="sq-card" style={{ padding: 16, display: 'flex', alignItems: 'center', gap: 14 }}>
              ...
              <div style={{ flex: 1 }}>
                <div className="sq-display" style={{ fontSize: 15, fontWeight: 600 }}>Court {courtNo} · {court?.type}</div>
                <div className="sq-mono" style={{ fontSize: 11.5, color: 'var(--sq-text-2)', marginTop: 2 }}>Today · {time} – {endTime} · 60 min</div>
              </div>
                ...
                {METHODS.map((m) => {
                  ...
                  return (
                    <button key={m.id} onClick={() => setMethod(m.id)} style={{
                      textAlign: 'left', cursor: 'pointer', padding: '14px 16px', borderRadius: 13, display: 'flex', alignItems: 'center', gap: 13,
                      background: on ? 'color-mix(in srgb, var(--sq-gold) 10%, var(--sq-surface))' : 'var(--sq-surface)',
                      border: '1.5px solid ' + (on ? 'color-mix(in srgb, var(--sq-gold) 50%, transparent)' : 'var(--sq-border)'),
                    }}>
                      <span style={{ width: 30, display: 'flex', justifyContent: 'center', color: on ? 'var(--sq-gold)' : 'var(--sq-text-2)' }}>
                        {m.id === 'applepay' ? <ApplePayMark /> : m.id === 'telda' ? <TeldaMark /> : <Icons.Wallet size={20} />}
                      </span>
            ...
            <p style={{ margin: 0, fontSize: 11.5, color: 'var(--sq-text-3)', lineHeight: 1.5, display: 'flex', gap: 7 }}>
              <Icons.Lock size={13} /> Payments are encrypted. You can cancel free up to 6 hours before your slot.
            </p>
          </div>
        )}
      </MScreen>
...
function ApplePayMark() {
  return <span style={{ fontFamily: 'system-ui, sans-serif', fontWeight: 600, fontSize: 15, display: 'inline-flex', alignItems: 'center' }}> Pay</span>;
}
function TeldaMark() {
  return <span style={{ fontFamily: 'var(--sq-display)', fontWeight: 700, fontSize: 13, letterSpacing: '-0.02em' }}>telda</span>;
}
